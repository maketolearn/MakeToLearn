...
const Submission = () => {
    ...
    const [fabricationGuidePackage, setFabricationGuidePackage] = useState()
    ...
    async function createDataset() {
        ...
        // Need to parse through multi-field entries for different areas below
        // Multiple authors
        ...
        const res = await axios.post(BACKEND_URL, 
            ...
            .then(data => {
                ...
                setDoi(doi)
                if(!fabricationGuidePackage && !instructResourcePackage && !thumbnailImage) {
                    navigate(`/success`)
                }
            })
            .catch(error => {
        ...
        Promise.all(promises).then(() => {
            ...
            zip.generateAsync({ type: "blob" }).then(function (mainZipContent) {
                if(prefix === "Fabrication_") {
                    setFabricationGuidePackage(mainZipContent)
    ...
    async function uploadFiles(doi) {
        try {
            ...
            if(fabricationGuidePackage) {
                const fabricationFormData = new FormData()
                fabricationFormData.append("file", fabricationGuidePackage)
                fabricationFormData.append("fileName", fabricationGuidePackage.name)
                const fabricationResult = await axios.post(BACKEND_URL, 
                    fabricationFormData, {
                        headers: headers
                    })
                    .then(data => {})
            ...
            if(instructResourcePackage) {
                const instructFormData = new FormData()
                instructFormData.append("file", instructResourcePackage)
                instructFormData.append("filename", instructResourcePackage.name)
                const instructResult = await axios.post(BACKEND_URL, 
                    instructFormData, {
                        headers: headers
                    })
                    .then(data => {})
            ...
            if(thumbnailImage) {
                ...
                thumbnailFormData.append("filename", thumbnailImage.name)
                const thumbnailResult = await axios.post(BACKEND_URL, 
                    thumbnailFormData, {
                        headers: headers
                    })
                    .then(data => {})
        ...
        <div>
            <body>
                <div className="site">
                    ...
                    { loggedIn ?
                        <div id="page">
                            <p>Congratulations on successfully logging into the CAD Library!</p>
                            <ul>
                                ...
                                <br></br>
                                <li>The information requested about the object in the form below is described in the article:</li><a href="https://citejournal.org/volume-23/issue-3-23/objects-to-think-with/metadata-standards-for-educational-objects/">Metadata Standards for Educational Objects</a>
                            </ul>
                            <p><i>Please contact a curator if you have any questions or would like to discuss this.</i></p>
                            ...
                            <form>
                                <table>
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><label for="title"> <b className="req">Name of Object</b><span className="toolTip" title={tooltips.title}>?</span></label></td>
                                            <td><input id="title" cols="40" rows="1" type="text"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><label for="discourseLink"> <b className="req">Link to CAD Library Forum Thread</b><span className="toolTip" title={tooltips.discourseLink}>?</span></label></td>
                                        ...
                                        <tr>
                                            <td><b className="req">Point of Contact</b><span className="toolTip" title={tooltips.pointOfContact}>?</span></td>
                                        ...
                                        <tr>
                                            <td><b><label for="sampleLearningGoals">Sample Learning Goal<span className="toolTip" title={tooltips.sampleLearningGoals}>?</span></label></b></td>
                                            <td><input id="sampleLearningGoals" type="text" placeholder="Sample learning goal"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label for="contentAlignment">Content Standard<span className="toolTip" title={tooltips.contentAlignment}>?</span></label></b></td>
                                            <td><input id="contentAlignment" type="text" placeholder="e.g. CCSS.MATH.CONTEXT.6.NS.A.1"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label className="req" for="gradeLevels">Grade Levels<span className="toolTip" title={tooltips.gradeLevels}>?</span></label></b></td>
                                            <td>
                                                <div class="grade-checkboxes">
                                                    {grades.map((grade) => 
                                                        <label id="checkbox" key={grade}><input className="subject-filter" type="checkbox" checked={gradesSelected.includes(grade)} onChange={() => handleGradeChange(grade)}></input>{grade}</label>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td><b><label for="cadFormat">CAD Format<span className="toolTip" title={tooltips.cadFormat}>?</span></label></b></td>
                                            <td><input id="cadFormat" type="text"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label for="consumableCost">Estimated Material Cost (Consumable)<span className="toolTip" title={tooltips.consumableCost}>?</span></label></b></td>
                                            <td><input id="consumableCost" type="text" placeholder="e.g., 10.5, 20, etc"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label for="reusableCost">Estimated Material Cost (Reusable)<span className="toolTip" title={tooltips.reusableCost}>?</span> </label></b></td>
                                            <td><input id="reusableCost" type="text" placeholder="e.g., 10.5, 20, etc"/></td>
                                        </tr>
                                        <br />

                                        <tr>
                                            <td><b><label for="equipment">Fabrication Equipment<span className="toolTip" title={tooltips.equipment}>?</span></label></b></td>
                                            <td><input id="equipment" type="text"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label for="fabricationTime">Fabrication Time<span className="toolTip" title={tooltips.fabricationTime}>?</span></label></b></td>
                                            <td><textarea id="fabricationTime" cols="40" rows="2" type="text" placeholder="to the nearest tenth of an hour; i.e., 1.5 hours"/></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><b><label for="assemblyTime">Assembly Time<span className="toolTip" title={tooltips.assemblyTime}>?</span></label></b></td>
                                        ...
                                        <tr>
                                            <td><b><label for="objectType">Object Type<span className="toolTip" title={tooltips.objectType}>?</span></label></b></td>
                                            <td>
                                        ...
                                        <tr>
                                            <td><label for="fabGuidePackage"><b className="req">Fabrication Guide</b><span className="toolTip" title={tooltips.fabGuidePackage}>?</span></label></td>
                                            <td><input type="file" onChange={(event) => handleZipping(event, 'Fabrication_')} multiple></input></td>
                                        </tr>
                                        <br></br>

                                        <tr>
                                            <td><label for="instructionalResourcesPackage"><b className="req">Instructional Resources</b><span className="toolTip" title={tooltips.instructionalResourcesPackage}>?</span></label></td>
                                        ...
                                        <br></br>
                                        <p>Please upload a thumbnail image for your object as a .png, .jpg, or .jpeg file.</p>

                                        <tr>
                                            <td><label for="thumbnailImage"> <b className="req">Thumbnail Image</b><span className="toolTip" title={tooltips.thumbnailImage}>?</span></label></td>
                        ...
                        :
                        <div id="page">
                            <p>Before submitting an object for review:</p>
                            <ol>
                            ...
                            <p><button className="button" onClick={discourseAuth}> Log in to the CAD Library Forum </button></p>
                        </div>
                    }
                </div>
            </body>
